...
01-fundamentals/
│
├── 05-measurement-and-instrumentation-fundamentals.md
├── 06-sensors-and-transducers.md   ← this file
│
└── assets/
    └── 06-sensors-and-transducers/
        ...
        │   ├── strain-gauge.svg
        │   ├── pt100-sensor.jpg
        │   ├── inductive-proximity-sensor.jpg
        │   ├── hall-effect.png
        │   ├── photoelectric-sensor.png
        │   ├── thermocouple-k-type.jpg
        │   ├── rotary-encoder.jpg
        │   └── conveyor-line.jpg
        │
        └── animations/
            ├── capacitive-sensing.html      (plates moving closer)
            ├── inductive-proximity.html     (field + eddy currents + target)
            ├── photoelectric-beam.html      (object interrupting the beam)
            ├── hall-effect.html             (charge deflection under a field)
            └── sensor-to-plc-loop.html      (full signal journey animated)
```

**Notes for building this out on GitHub:**
- The Mermaid diagram in Section 29 renders natively on GitHub — no image export needed.
- Every photograph above is hot-linked to Wikimedia Commons via its stable `Special:FilePath` redirect, with license and credit noted beneath each image. For a fully offline repository, download the originals into `/images` and switch to relative paths.
- Keep this chapter focused on *how sensing and transduction work*. PLC analog scaling, actuator selection, and closed-loop control belong in later chapters — repeating them here would break the logical progression of the `01-fundamentals` sequence.
- Reserve `.html` files in `/animations` for the mechanisms where motion genuinely explains the physics: inductive field disturbance, capacitive plate spacing, and the photoelectric beam interruption are the three highest-value animations in this chapter.
